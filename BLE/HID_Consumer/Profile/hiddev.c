...
static void hidDevPairStateCB(uint16_t connHandle, uint8_t state, uint8_t status)
{
    ...
    else if(state == GAPBOND_PAIRING_STATE_BONDED)
    {
        if(status == SUCCESS)
        {
            ...
            PRINT("GAPBondMgr_ResolveAddr result:%d\r\n",result);
//            PRINT("public device address:");
//            hex_dump(addr, 6);
            Bt_Par_Act.Link_Type = peer_addr_type;
            tmos_memcpy(Bt_Par_Act.Ble_LinkMac, addr, 6);
            PRINT("public device address:%02x,mac:%02X %02X %02X %02X %02X %02X\r\n",Bt_Par_Act.Link_Type,
                    Bt_Par_Act.Ble_LinkMac[0],
                    Bt_Par_Act.Ble_LinkMac[1],
                    Bt_Par_Act.Ble_LinkMac[2],
                    Bt_Par_Act.Ble_LinkMac[3],
                    Bt_Par_Act.Ble_LinkMac[4],
                    Bt_Par_Act.Ble_LinkMac[5]
            );

            Bt_Par_Act.Ble_Sta.Bit.Ble_Link_Sta = 1;
            Bt_Par_Act.Ble_Sta.Bit.Ble_LinkMac_Sta = 1;
...
//            tmos_start_task(Peripheral_TaskID, SBP_PERIODIC_EVT, SBP_PERIODIC_EVT_PERIOD);
    ...
    {
        PRINT("GAPBOND_PAIRING_STATE_BOND_SAVED\r\n");
        result =  GAPBondMgr_ResolveAddr(peer_addr_type,peer_addr,addr);
        PRINT("GAPBondMgr_ResolveAddr result:%d\r\n",result);
//        PRINT("public device address:");
//        hex_dump(addr, 6);
        Bt_Par_Act.Link_Type = peer_addr_type;
        tmos_memcpy(Bt_Par_Act.Ble_LinkMac, addr, 6);
        PRINT("public device address:%02x,mac:%02X %02X %02X %02X %02X %02X",Bt_Par_Act.Link_Type,
                Bt_Par_Act.Ble_LinkMac[0],
                Bt_Par_Act.Ble_LinkMac[1],
                Bt_Par_Act.Ble_LinkMac[2],
                Bt_Par_Act.Ble_LinkMac[3],
